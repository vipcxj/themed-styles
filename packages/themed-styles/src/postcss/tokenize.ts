...
interface Input {
    ...
    error: (message: string, line: number, column: number) => any;
}
...
export default function tokenizer (input: Input, options: { ignoreErrors?: boolean } = {}) {
    ...
    function nextToken (opts?: { ignoreUnclosed?: boolean }) {
        ...
        switch (code) {
            ...
            case BACKSLASH:
                ...
                break;
            case DOLLAR:
                if (css.charCodeAt(pos + 1) === OPEN_CURLY) {
                    ...
                    currentToken = ['word', '${' + css.slice(pos + 2, next).trim() + '}',
                        line, pos - offset,
                        line, next - offset
                    ];
